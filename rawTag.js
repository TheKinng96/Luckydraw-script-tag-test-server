  ...
  `
	...
	spinButton.addEventListener('click', async () => {
		...
		try {
			...
			const { keyId, key } = await fetch(`PUBLIC_APP_URL/api/hmac?shop=${shopId}`).then((r) => {
...
;(function () {
	...
	function initLuckyDraw() {
		...
		const shopId = window.Colorme?.shop?.account_id
		fetch(`PUBLIC_APP_URL/api/wheel?shop-id=${shopId}`)
			.then((r) => {
	...
	async function checkIfShouldRefetch() {
		try {
			const shopId = window.Colorme?.shop?.account_id
			const r = await fetch(`PUBLIC_APP_URL/api/wheel/last-updated-at?shop-id=${shopId}`)
			if (!r.ok) throw new Error('Fetch failed')
